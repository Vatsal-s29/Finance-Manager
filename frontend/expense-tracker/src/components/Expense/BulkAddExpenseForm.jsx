...
const BulkAddExpenseForm = ({ onBulkAddExpense }) => {
    ...
    const validateExpenseEntry = (expense, index) => {
        if (!expense.category || !expense.amount || !expense.date) {
            throw new Error(
                `Entry ${
                    index + 1
                }: Missing required fields (category, amount, date)`
            );
        }
    ...
    const parseJSON = (data) => {
        try {
            ...
            return expenses.map((expense, index) => {
                validateExpenseEntry(expense, index);
                return {
                    icon: expense.icon || "",
    ...
    const handleFileUpload = (event) => {
        ...
        if (fileExtension === "csv") {
            ...
            Papa.parse(file, {
                ...
                complete: (results) => {
                    try {
                        const expenses = results.data.map((row, index) => {
                        ...
                        setShowPreview(true);
                        setBulkData(JSON.stringify(expenses, null, 2));
                        setParseMethod("json");
                    } catch (error) {
    ...
    const handlePreview = () => {
        try {
            ...
            if (parseMethod === "json") {
                ...
                expenses = results.data.map((row, index) => {
                    const expense = {
  ...
  },
  {
    ...
    const sampleCSV = `category,amount,date
Job,4500,2025-07-01
        ...
        <div className={`flex gap-6 ${showPreview ? "min-w-[900px]" : ""}`}>
            ...
            >
                ...
                <div className="flex space-x-3">
                    ...
                    >
                        Add {previewData.length} Expenses
                    </button>
                </div>
            </div>
...
export default BulkAddExpenseForm;
